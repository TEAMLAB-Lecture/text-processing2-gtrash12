...
def digits_to_words(input_string):
    ...
    * 만약 인풋 스트링에서 숫자가 존재하지 않는 다면, 빈 문자열 (empty string)을 반환함
    ...
    """
    digit_string = ""
    numsstrs = ["zero", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine"]
    for i in input_string :
        if i.isdigit() :
            digit_string = digit_string +" " +numsstrs[int(i)]
...
def to_camel_case(underscore_str):
    ...
    * 만약 어떤 변수 이름이 underscore로만 이루어 진다면, 빈 문자열만 반환해도 됨
    ...
    """
    camelcase_str = ""
    stripedstr = underscore_str.strip('_')
    do_upper = False
    for i in stripedstr :
        if i == '_' :
            do_upper = True
            ...
            if do_upper :
                ...
                nextchar = i.lower()
            camelcase_str = camelcase_str + nextchar
